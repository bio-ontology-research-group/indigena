...
@ck.option("--scoring_fct_norm", type=int, default=1, help="Norm for TransE scoring function (1 or 2)")
...
def main(fold, graph2, graph3, graph4, projector_name, mode, embedding_dim, 
    ...
    model_out_filename = f"data/models/projector_{projector_name}_model_transe_{mode}_fold_{fold}_seed_{random_seed}_dim_{embedding_dim}_bs_{batch_size}_lr_{learning_rate}_epochs_{num_epochs}_norm_{scoring_fct_norm}_{graph_status}.pt"
    inductive_results_out_file = f"data/results/kge_results_{projector_name}_transe_inductive_fold_{fold}_seed_{random_seed}_dim_{embedding_dim}_bs_{batch_size}_lr_{learning_rate}_epochs_{num_epochs}_norm_{scoring_fct_norm}_{graph_status}.tsv"
    transductive_results_out_file = f"data/results/kge_results_{projector_name}_transe_transductive_fold_{fold}_seed_{random_seed}_dim_{embedding_dim}_bs_{batch_size}_lr_{learning_rate}_epochs_{num_epochs}_norm_{scoring_fct_norm}_{graph_status}.tsv"
