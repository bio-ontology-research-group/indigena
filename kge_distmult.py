...
def main(fold, graph2, graph3, graph4, projector_name, mode, embedding_dim, 
    ...
    model_out_filename = f"data/models/projector_{projector_name}_model_distmult_{mode}_fold_{fold}_seed_{random_seed}_dim_{embedding_dim}_bs_{batch_size}_lr_{learning_rate}_epochs_{num_epochs}_{graph_status}.pt"
    inductive_results_out_file = f"data/results/kge_results_{projector_name}_distmult_inductive_fold_{fold}_seed_{random_seed}_dim_{embedding_dim}_bs_{batch_size}_lr_{learning_rate}_epochs_{num_epochs}_{graph_status}.tsv"
    transductive_results_out_file = f"data/results/kge_results_{projector_name}_distmult_transductive_fold_{fold}_seed_{random_seed}_dim_{embedding_dim}_bs_{batch_size}_lr_{learning_rate}_epochs_{num_epochs}_{graph_status}.tsv"
