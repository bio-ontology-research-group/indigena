...
def perturbCsv = System.getenv("PERTURB_DISEASE_CSV")
if (perturbCsv) {
    logger.info("Overriding disease2pheno from perturbation file: ${perturbCsv}")
    disease2pheno = new HashMap()
    new File(perturbCsv).eachLine { line, idx ->
        if (idx > 1) {
            def parts = line.split(",")
            def disease = parts[0]
            def phenotype = parts[1]
            if (existingHpPhenotypes.contains(phenotype)) {
                if (! disease2pheno.containsKey(disease)) disease2pheno[disease] = []
                disease2pheno[disease].add(phenotype)
            }
        }
    }
    logger.info("Overridden disease2pheno size: ${disease2pheno.size()}")
}
...
def _ptag = System.getenv("PERTURB_TAG") ? ("_" + System.getenv("PERTURB_TAG")) : ""
def out_file = rootDir + "/baseline_results/${icMeasure}_${pairwiseMeasure}_${groupwiseMeasure}_fold${options.fold}${_ptag}_results.txt"
