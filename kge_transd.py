...
def model_resolver(triples_factory, embedding_dim, relation_dim, random_seed):
    model = TransD(
...
@ck.option("--relation_dim", type=int, default=50, help="Embedding dimension for relations")
...
def main(fold, graph2, graph3, graph4, projector_name, mode, embedding_dim, 
         relation_dim, batch_size, learning_rate, num_epochs, random_seed, 
    ...
    if no_sweep:
        ...
        relation_dim = wandb.config.relation_dim
    ...
    model = model_resolver(triples_factory, embedding_dim, relation_dim, random_seed).to("cuda")

    graph_status = "graph4" if graph4 else "graph3" if graph3 else "graph2" if graph2 else "graph1"
    
    model_out_filename = f"data/models/projector_{projector_name}_model_transd_{mode}_fold_{fold}_seed_{random_seed}_dim_{embedding_dim}_rel_{relation_dim}_bs_{batch_size}_lr_{learning_rate}_epochs_{num_epochs}_{graph_status}.pt"
    inductive_results_out_file = f"data/results/kge_results_{projector_name}_transd_inductive_fold_{fold}_seed_{random_seed}_dim_{embedding_dim}_rel_{relation_dim}_bs_{batch_size}_lr_{learning_rate}_epochs_{num_epochs}_{graph_status}.tsv"
    transductive_results_out_file = f"data/results/kge_results_{projector_name}_transd_transductive_fold_{fold}_seed_{random_seed}_dim_{embedding_dim}_rel_{relation_dim}_bs_{batch_size}_lr_{learning_rate}_epochs_{num_epochs}_{graph_status}.tsv"
